...
let removecard = (id)=>{
let updatecard = product.filter((item)=>{return item.id!==id})
...
console.log(dec);
  ...
  return (
    <div className='p-6 gap-5'>
      <div className='`h-12.5 text-center text-[30px] font-black border-[3px]'>Product List</div>
       <div className='flex-wrap flex gap-10 justify-center items-center'>
     {
      product.map((elem)=>{
      return  <Components key={elem.id} values={elem} update={removecard} add={increaseQnty} sub={decreaseQnty} />
      })
     }
       </div>
    </div>
